...
\begin{itemize}
  ...
  \item \textbf{Pattern Matching: Polynomier}
\end{itemize}
...
\TODO{Hvordan vælger vi $p$?}

Nu beregner vi så $a \bmod p$. Derudover ønsker vi at beregne alle $B_j \bmod p$ i $O(n)$ tid, hvilket vi både udregner i baglæns rækkefølge og hvor vi aflæser deres bitstreng baglæns.
...
\subsection{Pattern Matching: Polynomier}
...
Nu ønsker vi at bestemme $A(x) \? B_j(x)$, hvilket svarer til at de to bitstrenge er ens.
...
\TODO{Spørg til hvad der er fidusen ved denne. At vi skal kommunikere få bits eller lav køretid? Spørg til om vi tager udgangspunkt i $B$ er $n$ eller $m$ langt, og om vi kigger på $B$ eller $B_j$.}
